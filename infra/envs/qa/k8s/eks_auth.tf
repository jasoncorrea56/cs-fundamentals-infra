############################################################
# EKS aws-auth & GitHub deployer RBAC (qa/k8s layer)
#
# - Maps:
  ...
  data = merge(
    {
      mapRoles = yamlencode(concat([
        {
          # Worker nodes (from qa/aws remote state)
          rolearn  = data.terraform_remote_state.qa_aws.outputs.eks_node_role_arn
        ...
        {
          # GitHub deployer IAM role (from qa/aws remote state)
          rolearn  = data.terraform_remote_state.qa_aws.outputs.github_actions_role_arn
